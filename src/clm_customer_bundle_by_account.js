...
exports.handler = (event, context, callback) => {
    get_account_bundle(event, process.env, callback);
};
 ...
 */
function get_account_bundle(request, envVar, callback) {
    validator.validate_request(request,envVar).then(valid => {
      try {
        //call backend only with accountNumber
          ...
          callback(null, response_handler.prepare_account_bundle_response(api_response));
      } catch (e) {
